...
class ShiftAssignmentController extends Controller
{
    ...
    public function staffCheckin(Request $request)
    {
        try {
            ...
            // nếu hệ thống khóa checkin và nhân viên không phải master thì không cho phép checkin
            if($systemConfigs[SystemConfigKey::ENABLE_LOCK_STAFF_CHECKIN->value] == 1 && $staff->is_master == 0){
                return response()->json([
                    'status' => 'error',
            ...
            if ($waitingStaffCount == 0) {
                ...
                $gateShift->update(['queue_status' => GateShift::QUEUE_STATUS_RUNNING]);
            }
        ...
        try {
            ...
            try {
                ...
                if ($remainingCheckinStaff === 0) {
                    GateShift::where('id', $assignment->gate_shift_id)([
